...
static int spin_interrupt_release(struct inode *inode, struct file* file){
	...
	printk("spin interrupt : show all\n");
...
{
	int ret;
	printk("simple_sensor : Init Module\n");
	...
	if(ret)
	{
		printk("simple_sensor : Unalbe to request IRQ : %d\n", irq_num);
...
static void __exit spin_interrupt_exit(void)
{
	...
	printk("spin_interrupt : Exit module\n");
	cdev_del(cd_cdev);
